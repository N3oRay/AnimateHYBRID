...
# -------------------------
# Sauvegarde frame (PIL) sécurisée
# -------------------------
...
def main(args):
    ...
    print(f"📌 Paramètres de génération : fps={fps}, frames/image={num_fraps_per_image}, steps={steps}, guidance={guidance_scale}, seed={seed}")
    print(f"⏱ Durée totale estimée : {total_frames/fps:.1f}s")
    ...
    # VAE
    # -------------------------
    vae = safe_load_vae(cfg.get("vae_path") or args.pretrained_model_path,
                        device=device, fp16=args.fp16, offload=args.vae_offload)
    ...
    with open(csv_file, "w", newline="") as f_csv:
        ...
        for img_path in input_paths:
            ...
            if args.vae_offload:
                vae.to(device)
                if args.fp16 and device=="cuda": vae = vae.half()
            ...
            for f_idx in range(num_fraps_per_image):
                ...
                # Convert for PIL + OpenCV (uint8)
                frame_array = (frame_tensor.clamp(-1,1)+1)/2
                frame_array = np.nan_to_num(frame_array.cpu().permute(0,2,3,1)[0].numpy(),
                                            nan=0.0, posinf=1.0, neginf=0.0)
                frame_array_uint8 = (np.clip(frame_array,0,1)*255).astype(np.uint8)

                # Init video writer
                if video is None:
                    h, w = frame_array_uint8.shape[:2]
                    video_path = output_dir / "animation.mp4"
                    video = cv2.VideoWriter(str(video_path),
                                            cv2.VideoWriter_fourcc(*'mp4v'),
                                            fps, (w,h))

                video.write(cv2.cvtColor(frame_array_uint8, cv2.COLOR_RGB2BGR))

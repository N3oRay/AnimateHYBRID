...
class Pose:
    ...
    def create_upper_body_mask(
        ...
        if debug and debug_dir is not None:
            ...
            mask_debug = cv2.resize(mask_np_debug, (W*4, H*4), interpolation=cv2.INTER_NEAREST)
            mask_debug_rgb = cv2.cvtColor(mask_debug, cv2.COLOR_GRAY2BGR)

            save_path = os.path.join(debug_dir, f"torso_mask_PRO_{frame_counter:05d}.png")
            cv2.imwrite(save_path, mask_debug_rgb)

            print(f"[DEBUG] Torso mask PRO saved: {save_path}")
    ...
    def create_upper_body_mask_v2(
        ...
        if debug and debug_dir is not None:
            ...
            save_path = os.path.join(debug_dir, f"torso_mask_{frame_counter:05d}.png")
            cv2.imwrite(save_path, mask_debug_rgb)

            print(f"[DEBUG] Torso mask (rounded) saved: {save_path}")
...
def apply_face_warp_v2(
    ...
    """
    Warp du visage avec micro-expressions, respiration et vrai sourire.
    ...
    # =========================
    # Base motion visage (respiration + micro-mouvements)
    # =========================
    face_delta = torch.zeros((B,1,1,2), device=device)
    face_delta[...,1] += 0.03  * torch.sin(t_resp_y)
    face_delta[...,0] += 0.015 * torch.sin(t_resp_x)
    face_delta[...,0] += 0.006 * torch.sin(t_micro)
    face_delta[...,1] += 0.009 * torch.sin(t_micro * 1.5)

    # =========================
    # Préparer masque bouche
    # =========================
    ...
    # =========================
    # Vrai sourire (coins de la bouche)
    # =========================
    face_delta_expanded = face_delta.expand(B,H,W,2).clone()

    # Conversion coins en pixels
    ...
    # Appliquer sur le masque de la bouche
    face_delta_expanded[...,0] += (delta_left[...,0] + delta_right[...,0]) * mouth_mask_exp[...,0]
    face_delta_expanded[...,1] += (delta_left[...,1] + delta_right[...,1]) * mouth_mask_exp[...,0]

    # =========================
    # Respiration bouche supplémentaire
    # =========================
    ...
    if debug:
        ...
        print("[DEBUG] Face warp v3 applied: smile, respiration, micro-mouvements, vent")
...
def apply_face_warp_v1(
    ...
    """
    Warp du visage avec micro-expressions et respiration amplifiées.
    Gestion séparée du mouvement global et local pour plus de réalisme.
    """
    if device is None:
        device = latents.device
    ...
    # Temps
    # =========================
    t_resp_y = frame_counter / 8.0
    t_resp_x = frame_counter / 10.0
    t_micro  = frame_counter / 5.0
    t_wind1 = frame_counter / 15.0
    t_wind2 = frame_counter / 60.0
    t_smile = frame_counter / 8.0
    t_breath = frame_counter / 12.0

    # =========================
    # Delta global (toute la tête)
    # =========================
    face_delta = torch.zeros((B,1,1,2), device=device)
    face_delta[...,1] += 0.03  * torch.sin(torch.tensor(t_resp_y, device=device))
    face_delta[...,0] += 0.015 * torch.sin(torch.tensor(t_resp_x, device=device))
    face_delta[...,0] += 0.006 * torch.sin(torch.tensor(t_micro, device=device))
    face_delta[...,1] += 0.009 * torch.sin(torch.tensor(t_micro*1.5, device=device))

    # =========================
    # Delta bouche / micro expressions
    # =========================
    smile_delta = 0.08 * torch.sin(torch.tensor(t_smile, device=device))
    breath_delta = 0.018 * torch.sin(torch.tensor(t_breath, device=device))
    ...
    # =========================
    # Effet vent / micro-oscillation globale
    # =========================
    wind_delta_x = 0.004 * torch.sin(torch.tensor(t_wind1, device=device))
    wind_delta_y = 0.003 * torch.sin(torch.tensor(t_wind2, device=device))
    face_delta_expanded[...,0] += mask_face_exp[...,0] * wind_delta_x
    face_delta_expanded[...,1] += mask_face_exp[...,0] * wind_delta_y
    ...
    # =========================
    if debug:
        save_impact_map(latents_out, latents_in, debug_dir, frame_counter, prefix="face_v3")
        print("[DEBUG] Face warp v3 applied: amplified, micro expressions + vent")

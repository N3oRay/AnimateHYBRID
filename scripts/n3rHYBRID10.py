...
def save_frame(img_array, filename):
    """Enregistre une frame au format PNG, sécurité NaN/Inf incluse."""
...
def encode_image_latents(image_tensor, vae, scale=LATENT_SCALE, dtype=torch.float16):
    ...
    return latents.unsqueeze(2)  # ajoute dim pour time/frame
...
def main(args):
    ...
    print(f"📌 Paramètres : fps={fps}, frames/image={num_fraps_per_image}, steps={steps}, seed={seed}")
    ...
    with open(csv_file, "w", newline="") as f_csv:
        ...
        for img_path in input_paths:
            ...
            for f_idx in range(num_fraps_per_image):
                ...
                # -------------------------
                frame_array = frame_tensor.permute(0,2,3,1)[0].cpu().numpy()
